...
@app.route('/manage/files/list')
@root_required
def file_manager_list():
    path = request.args.get('path', '')
    local_path = safe_join(app.config['FILE_DIRECTORY'], path)

    dirs = []
    files = []
    
    try:
        dir_listing = os.listdir(local_path)
        dir_listing.sort()
        for name in dir_listing:
            full = safe_join(local_path, name)
            element = { "name" : name, "path" : safe_join(path, name) }
            if os.path.isdir(full):
                dirs.append(element)
            else:
                element["size"] = os.path.getsize(full)
...
@app.route('/manage/files/mkdir')
@root_required
def file_manager_mkdir():
    path = request.args.get('path', '')
    try:
        local_path = safe_join(app.config['FILE_DIRECTORY'], path)
        os.mkdir(local_path)
...
@app.route('/manage/files/move')
@root_required
def file_manager_move():
    #TODO: write it
    pass
